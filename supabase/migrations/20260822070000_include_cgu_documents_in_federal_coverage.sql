...
-- A CGU publica dois recortes complementares: um retrato agregado por emenda
-- e arquivos anuais por documento de empenho, liquidacao e pagamento. Eles
-- permanecem em colunas separadas para que cobertura nao seja confundida com
-- soma financeira nem para que um ano documental seja declarado vazio apenas
-- porque nao aparece no retrato agregado.
create or replace function api.get_public_federal_transfer_source_coverage()
returns table (
  source_key text,
...
with requested_years as (
  ...
  where (data_source.slug, source_endpoint.slug) in (
    ...
    ('cgu-portal-transparencia', 'federal-amendment-documents-open-data'),
  ...
  select
    document.archive_year as fiscal_year,
    count(*)::integer as record_count
  from territory.cgu_federal_amendment_documents as document
  group by document.archive_year
), document_partitions as (
  select
  ...
  join endpoints as endpoint
    on endpoint.source_slug = 'cgu-portal-transparencia'
   and endpoint.endpoint_slug = 'federal-amendment-documents-open-data'
  left join source.collection_partitions as partition
   ...
   and partition.partition_key =
     'federal-amendment-documents:' || requested.fiscal_year::text || ':2903201'
...
), document_coverage as (
  select
    'cgu_documents'::text as source_key,
    ...
    pg_catalog.replace(
      partition.base_url,
      '{year}',
      partition.fiscal_year::text
    ) as source_url
  from document_partitions as partition
  left join document_counts as counts
...
select
  ...
  coverage.source_url,
  'federal-transfer-source-coverage/2.0.0'::text as methodology_version
from (
  select * from cgu_coverage
  union all
  select * from document_coverage
  union all
...
comment on function api.get_public_federal_transfer_source_coverage() is
  'Cobertura anual sanitizada de quatro series federais, mantendo retrato agregado e documentos da CGU separados.';
...
values (
  'administrator',
  'migration:include-cgu-documents-in-federal-coverage',
  'methodology.federal_transfer_source_coverage_corrected',
  ...
  jsonb_build_object(
    ...
    'financial_series_kept_separate', true,
