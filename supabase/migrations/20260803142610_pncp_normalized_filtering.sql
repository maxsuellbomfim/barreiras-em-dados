-- Chave compartilhada com o catálogo de sugestões. O texto da fonte continua
-- sendo retornado; somente a comparação dos filtros usa a chave.

create or replace function api.pncp_label_key(value text)
returns text
language sql
immutable
strict
...
revoke all on function api.pncp_label_key(text) from public, anon, authenticated;
...
comment on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text) is
  'Contratações PNCP filtráveis por chave normalizada de modalidade, situação e unidade.';
