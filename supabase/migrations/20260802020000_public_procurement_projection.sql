-- Licitações públicas: projeção fiel do registro oficial do PNCP.
-- Nenhum valor é calculado aqui: totais estimados e homologados são os
-- informados pelo próprio portal. Documento de fornecedor pessoa física
-- (CPF) nunca é exposto; CNPJ de pessoa jurídica é público por natureza.

create or replace function api.get_pncp_procurements(
...
begin
  ...
  from (
    select distinct on (record.payload ->> 'numeroControlePNCP')
      ...
      case
        when record.payload ->> 'valorTotalEstimado'
            ~ '^-?[0-9]+(\.[0-9]+)?$'
        then (record.payload ->> 'valorTotalEstimado')::numeric
      end as valor_estimado,
      case
        when record.payload ->> 'valorTotalHomologado'
            ~ '^-?[0-9]+(\.[0-9]+)?$'
        then (record.payload ->> 'valorTotalHomologado')::numeric
      end as valor_homologado,
      case
        when record.payload ->> 'dataPublicacaoPncp'
            ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}'
        then left(record.payload ->> 'dataPublicacaoPncp', 10)::date
  ...
  left join lateral (
    ...
    from (
      select distinct on (
        (winner.payload ->> 'numeroItem'),
        (winner.payload ->> 'sequencialResultado')
      )
        ...
        jsonb_build_object(
          ...
          'ni_fornecedor',
          case
            when winner.payload ->> 'tipoPessoa' = 'PJ'
            then winner.payload ->> 'niFornecedor'
          end,
          'valor_total_homologado',
          case
            when winner.payload ->> 'valorTotalHomologado'
                ~ '^-?[0-9]+(\.[0-9]+)?$'
            then (winner.payload ->> 'valorTotalHomologado')::numeric
          end,
          'data_resultado',
          case
            when winner.payload ->> 'dataResultado'
                ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}'
            then left(winner.payload ->> 'dataResultado', 10)
          end
        ) as item
      from raw.raw_records as winner
      where winner.record_type = 'pncp_resultado'
        and winner.payload ->> 'numeroControlePNCPCompra'
            = contratacao.control_number
  ...
  order by contratacao.data_publicacao desc nulls last,
    contratacao.control_number
  limit page_size;
end;
$function$;

revoke all on function api.get_pncp_procurements(integer) from public;
grant execute on function api.get_pncp_procurements(integer)
  to anon, authenticated;

comment on function api.get_pncp_procurements(integer) is
  'Contratações de Barreiras no PNCP com resultados homologados; valores '
  'são os informados pelo portal, sem cálculo próprio; CPF nunca exposto.';
