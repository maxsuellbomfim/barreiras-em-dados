...
-- Mantém o contrato anterior disponível durante o deploy e cria uma leitura
-- filtrada. O ranking e o resumo financeiro continuam em RPCs separadas e
-- sempre representam o exercício inteiro, não o resultado desta busca.

create function api.get_public_bahia_state_loa_study_filtered(
...
revoke all on function api.get_public_bahia_state_loa_study_filtered(
  smallint, integer, integer, text, text, text
) from public;
grant execute on function api.get_public_bahia_state_loa_study_filtered(
  smallint, integer, integer, text, text, text
...
comment on function api.get_public_bahia_state_loa_study_filtered(
  ...
  'Pesquisa e pagina as autorizacoes estaduais de Barreiras; totais filtrados e acervo anual permanecem distintos, sem recalcular ranking ou somar estagios.';
