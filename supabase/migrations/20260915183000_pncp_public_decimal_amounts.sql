...
-- Corrige apenas a leitura dos três valores monetários já publicados no PNCP.
-- [.] representa o ponto literal sem depender de escapes de strings SQL.
-- Sem conversão de moeda ou arredondamento; dados brutos, filtros, tipos e
-- permissões permanecem preservados.
create or replace function api.get_pncp_procurements_normalized(
...
revoke all on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text) from public, anon, authenticated;
...
comment on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text) is
  'Contratações PNCP filtráveis com itens, resultados e execução financeira ligados por identificador oficial.';
