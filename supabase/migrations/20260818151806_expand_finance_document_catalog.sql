...
-- Amplia o catalogo publico de documentos financeiros com dois recursos do
-- portal municipal descobertos e verificados em 18/08/2026: as listagens de
-- transferencias concedidas (pdc-convenios-transferencias-realizadas) e o
-- acervo de obras/prestacao de contas (pdc-obras-pdc). Mesmo contrato de
-- catalogo dos demais pdc-*: titulo, periodo, PDF oficial e hash preservado;
-- nenhum valor e extraido ou somado.
...
begin
  ...
  select
    ...
    'public-finance-documents/1.5.0',
...
comment on function api.get_public_finance_documents(integer, text) is
  'Catálogo financeiro com transferências concedidas e obras/PDC; vínculo exato entre registro da API e PDF preservado. Não calcula saldo de dívida.';
