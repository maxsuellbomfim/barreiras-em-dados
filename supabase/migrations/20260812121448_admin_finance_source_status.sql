-- Distingue documentos ainda nao processados de fontes que foram processadas,
-- mas nao contem a secao esperada ou terminam com a secao incompleta.
...
begin
  ...
  with documents as (
    select
      ...
      coalesce(latest.last_error_code, source_result.candidate_type)
        as latest_error_code,
      coalesce(latest.last_error_detail, source_result.detail)
        as latest_error_detail,
    ...
    left join lateral (
      select
        ...
        nullif(result.result_payload ->> 'detail', '') as detail
      from raw.extraction_jobs as job
  ...
  select
    ...
    documents.latest_error_code,
    documents.latest_error_detail,
    documents.published_rows,
    'finance-ingestion-inventory/1.2.0'::text
  from documents
...
comment on function api.get_finance_ingestion_inventory(integer, text, text) is
  'Inventario interno que distingue processamento pendente, secao ausente e documento oficial incompleto.';
