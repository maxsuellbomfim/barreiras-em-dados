-- Corrige a ambiguidade entre o parâmetro review_note e a coluna de mesmo
-- nome na função de revisão. A assinatura SQL permanece a mesma para o
-- cliente; apenas os nomes internos dos parâmetros mudam.

drop function if exists api.review_representative_alias_suggestion(uuid, text, text);

create function api.review_representative_alias_suggestion(
...
begin
  ...
  if p_review_decision not in ('accepted', 'rejected', 'needs_more_evidence') then
    raise exception 'decisão de alias inválida' using errcode = '22023';
  end if;
  if p_review_decision <> 'accepted'
     and nullif(btrim(p_review_note), '') is null then
  ...
  if p_review_decision = 'accepted' then
  ...
  update political.representative_alias_suggestions as suggestion_row
  set status = p_review_decision,
