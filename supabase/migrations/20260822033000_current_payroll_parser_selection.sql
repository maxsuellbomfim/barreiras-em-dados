...
create or replace function hr.get_pending_payroll_documents(
    ...
    where document.artifact_kind = 'document'
      ...
      and not exists (
        ...
        where job.raw_artifact_id = document.id
          and job.job_type = 'payroll_report_publication/1.3.0'
...
create or replace function hr.payroll_unresolved_document_count(
...
revoke all on function hr.get_pending_payroll_documents(
  integer, integer, integer, date
) from public;
revoke all on function hr.payroll_unresolved_document_count(date) from public;
grant execute on function hr.get_pending_payroll_documents(
  integer, integer, integer, date
) to collector_worker;
grant execute on function hr.payroll_unresolved_document_count(date)
  to collector_worker;

comment on function hr.get_pending_payroll_documents(
  integer, integer, integer, date
) is
  'Seleciona PDFs oficiais sem agregado produzido pelo parser vigente 1.4.0, sem ampliar permissoes das aplicacoes publicas.';
comment on function hr.payroll_unresolved_document_count(date) is
  'Conta PDFs oficiais sem agregado do parser vigente 1.4.0, sem expor dados brutos as aplicacoes publicas.';
