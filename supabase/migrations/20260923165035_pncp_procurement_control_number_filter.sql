...
-- Acrescenta o filtro exato por número de controle PNCP para a página própria
-- de cada contratação (/licitacoes/contratacao/<número>). O corpo é o mesmo
-- da migration 20260915183000; o novo parâmetro tem padrão nulo, então as
-- chamadas existentes continuam com o mesmo resultado. A troca de assinatura
-- exige drop/create para não deixar duas sobrecargas ambíguas no PostgREST.
...
create function api.get_pncp_procurements_normalized(
  ...
  control_number_filter text default null
)
returns table (
  control_number text,
  ano integer,
...
begin
  ...
  if control_number_filter is not null and length(trim(control_number_filter)) > 60 then
    raise exception 'control_number_filter muito longo' using errcode = '22023';
  end if;
  ...
  from (
    ...
    where record.record_type = 'pncp_contratacao'
      ...
      and (control_number_filter is null or record.payload ->> 'numeroControlePNCP' = trim(control_number_filter))
  ...
  left join lateral (
    ...
    from (
      select distinct on (current_item.external_item_number)
        current_item.external_item_number::bigint as numero_item,
        jsonb_build_object(
          'numero_item', current_item.external_item_number::bigint,
          'descricao', current_item.description,
          'quantidade', current_item.quantity,
          'unidade', current_item.unit_name,
          'valor_unitario_estimado', current_item.estimated_unit_amount,
          'valor_total', current_item.estimated_total_amount,
          'situacao', current_item.result_status,
          'catalogo_codigo', current_item.catalog_code
        ) as item
      from procurement.procurement_items as current_item
      join procurement.procurements as normalized_procurement
        on normalized_procurement.id = current_item.procurement_id
      where normalized_procurement.external_id = contratacao.control_number
        and not exists (
          select 1 from procurement.procurement_items as newer
          where newer.supersedes_id = current_item.id
        )
      order by current_item.external_item_number, current_item.version desc, current_item.created_at desc
    ) as entry
  ) as item on true
  left join lateral (
    select jsonb_agg(entry.item order by entry.numero_item) as lista
...
revoke all on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text, text) from public, anon, authenticated;
grant execute on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text, text) to anon, authenticated;

comment on function api.get_pncp_procurements_normalized(integer, text, smallint, text, text, text, text, text) is
  'Contratações PNCP filtráveis com itens, resultados e execução financeira ligados por identificador oficial; filtro exato por número de controle para a página própria.';
