...
-- Atos publicados a partir de edições com páginas transcritas por OCR passam
-- a informar a origem do texto. Nomes e números lidos por OCR podem conter
-- erro de leitura e a conferência literal compara o campo com o trecho da
-- mesma transcrição; o site precisa pedir conferência no documento oficial.
-- Corpo igual ao da migration 20260821011822 (MD5 conferido em produção),
-- acrescido da coluna text_source. A mudança do tipo de retorno exige
-- drop/create.
...
begin
  ...
  select
    ...
    deduped.review_mode,
    deduped.text_source,
    'approved-gazette-acts/1.7.0'::text
  from deduped
...
comment on function api.get_approved_gazette_acts(integer) is
  'Somente atos de pessoal aprovados, um por (tipo, portaria, data, pessoa), com revisão e evidência resolvidas em conjunto e origem do texto (embutido ou OCR).';
