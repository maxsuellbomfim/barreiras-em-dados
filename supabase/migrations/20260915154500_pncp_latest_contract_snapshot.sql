...
-- Corrige somente a seleção dos snapshots contratuais. Versões históricas,
-- linhagem, valores, território e permissões existentes são preservados.
...
begin
  ...
  for v_record in
    ...
    select snapshot.id, snapshot.payload, snapshot.payload_sha256
      ...
      left join lateral (
        select origin.payload_sha256
         ...
         where c.public_body_id = v_body_id
           and c.external_id = snapshot.external_id
     ...
     where current_contract.payload_sha256 is distinct from snapshot.payload_sha256
