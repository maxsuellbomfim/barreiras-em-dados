-- Fechamento mensal determinístico e inventário operacional do pipeline financeiro.
-- Nenhum total é produzido por IA: a função usa somente relatórios já validados
-- e preserva a distinção entre receita declarada e pagamento efetivado.
...
comment on function api.get_public_monthly_finance_closures(integer, smallint) is
  'Fechamento mensal determinístico: relatório de receita versus pagamento efetivado, com estado de cobertura explícito.';
...
comment on function api.get_finance_ingestion_inventory(integer, text, text) is
  'Inventário interno de PDFs financeiros preservados, com status de extração/publicação e último erro.';
