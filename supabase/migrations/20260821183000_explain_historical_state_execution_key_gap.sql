...
create or replace function api.get_public_state_amendment_source_coverage()
returns table (
...
select
  ...
  case
    ...
    when classified.fiscal_year between 2022 and 2025
      then 'blocked_missing_official_key'
  ...
  case when classified.loa_status = 'observed'
      and classified.fiscal_year not between 2022 and 2025
  ...
  'state-amendment-source-coverage/1.1.0'::text as methodology_version
from classified
...
comment on function api.get_public_state_amendment_source_coverage() is
  'Cobertura anual sanitizada: 2022-2025 explicita a ausencia de chave oficial comum sem fabricar ligacao ou valor de execucao.';
...
values (
  'administrator',
  'migration:explain-historical-state-execution-key-gap',
  'methodology.state_amendment_historical_key_gap_published',
  'api.get_public_state_amendment_source_coverage',
  gen_random_uuid(),
  jsonb_build_object(
    'methodology_version', 'state-amendment-source-coverage/1.1.0',
    'blocked_years', jsonb_build_array(2022, 2023, 2024, 2025),
    'execution_amounts_require_unique_official_key', true
  ),
  jsonb_build_object(
    'publishes_aggregate_counts_only', true,
    'publishes_personal_identifiers', false,
    'uses_name_similarity', false,
    'missing_link_is_not_zero', true
  )
);
