-- Compras: paginação de verdade na lista de contratações do PNCP. A função
-- atual só aceita page_size, e a página carregava 60 cartões de uma vez
-- (74 telas no celular). Esta é uma cópia de get_pncp_procurements_normalized
-- com page_offset; a ordenação (data de publicação, número de controle) já é
-- determinística, então as páginas não se sobrepõem nem pulam registros.

create function api.get_pncp_procurements_page(
...
revoke all on function api.get_pncp_procurements_page(integer, integer, text, smallint, text, text, text, text, text) from public, anon, authenticated;
grant execute on function api.get_pncp_procurements_page(integer, integer, text, smallint, text, text, text, text, text) to anon, authenticated;
