...
alter table source.pncp_contract_query_status
  drop constraint pncp_contract_query_status_state_check;
alter table source.pncp_contract_query_status
  add constraint pncp_contract_query_status_state_check check
  (state in ('unknown','pending','query_complete','empty_confirmed','inconclusive',
             'partial','interrupted','awaiting_source_publication'));
...
-- Rebuild only runs that have the new evidence-bearing observation.
-- Original runs, artifacts, coverage and retry queues are untouched.
do $$ declare target uuid; begin
  for target in select id from source.collection_runs
    where metrics->'control_plane'='true'::jsonb
      and metrics->'control_observations' @> '[{"state":"awaiting_source_publication"}]'::jsonb
