...
values (
  'barreiras-360',
  'Barreiras 360 — portal público',
  'Disponibilidade sintética das rotas públicas críticas do portal.',
  'secondary',
  false,
  'https://barreiras-em-dados.vercel.app/',
  'active',
  jsonb_build_object(
    'monitoring_scope', 'synthetic_critical_routes',
    'traffic_coverage', 'sampled_not_exhaustive',
    'public_statement', 'does_not_replace_vercel_request_logs'
  )
)
...
values (
  (select id from source.data_sources where slug = 'barreiras-360'),
  ...
  jsonb_build_object(
    ...
    'schedule', 'hourly_at_minute_17',
    'critical_route_count', 8,
    'minimum_daily_scheduled_runs', 20,
    'history_starts_at', statement_timestamp(),
    'scope_limitation', 'synthetic_samples_not_all_visitor_requests'
  )
)
...
set
  ...
  config = source.source_endpoints.config || excluded.config;

update source.source_endpoints as endpoint
set
  freshness_policy_kind = 'scheduled',
  freshness_expected_hours = 1,
  freshness_grace_hours = 1,
  freshness_policy_note =
    'Sondagem horária das rotas públicas críticas. É uma amostra e não substitui os logs integrais da Vercel.',
  freshness_policy_version = 'source-freshness/1.0.0'
from source.data_sources as data_source
where data_source.id = endpoint.data_source_id
  and data_source.slug = 'barreiras-360'
  and endpoint.slug = 'critical-public-pages';

create index collection_runs_public_availability_recent_idx
  on source.collection_runs (source_endpoint_id, started_at desc)
  where collector_version = 'public-availability-probe/1.0.0'
    and metrics ->> 'execution_origin' = 'github_actions';

create function api.get_collection_health_v8(
