...
begin
  ...
  with latest_complete_catalog as (
    ...
    from catalog_evidence as catalog
    join raw.raw_artifacts as origin_artifact
      on origin_artifact.collection_run_id = catalog.collection_run_id
    join raw.raw_records as record
      on record.raw_artifact_id = origin_artifact.id
    join source.source_endpoints as endpoint
  ...
  select
    ...
    'nonpayroll-workforce-coverage/1.0.0'::text
