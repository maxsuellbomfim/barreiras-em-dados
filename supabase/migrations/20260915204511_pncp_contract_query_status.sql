-- Latest-query projection only. Original executions and artifacts remain the ledger.
set local lock_timeout = '3s';
set local statement_timeout = '30s';
create table source.pncp_contract_query_status (
  control_number text primary key check (control_number ~ '^13654405000195-1-[0-9]{1,12}/[0-9]{4}$'),
  run_id uuid not null,
  run_started_at timestamptz not null,
  state text not null check (state in ('unknown','pending','query_complete','empty_confirmed','inconclusive','partial','interrupted')),
  checked_at timestamptz,
  evidence jsonb not null default '[]'::jsonb,
  updated_at timestamptz not null default statement_timestamp()
);
alter table source.pncp_contract_query_status enable row level security;
revoke all on source.pncp_contract_query_status from public, anon, authenticated, collector_worker;

create function source.refresh_pncp_contract_query_status(target_run uuid)
      ...
      if found then
        ...
        begin
          if (select count(*) from jsonb_array_elements(observations) where value->>'control'=key)<>1
            ...
            or coalesce(item->>'state','') not in ('query_complete','empty_confirmed','inconclusive','partial','interrupted')
          ...
          result_state := item->>'state'; evidence := item->'pages';
...
create function source.track_pncp_contract_query_status()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
  perform source.refresh_pncp_contract_query_status(new.id);
  return new;
end $$;
revoke all on function source.track_pncp_contract_query_status() from public, anon, authenticated, collector_worker;
create trigger pncp_contract_query_status_after_run
after insert or update of metrics,cursor_after,status,completed_at on source.collection_runs
for each row execute function source.track_pncp_contract_query_status();

-- One-time bounded-per-run normalization; public reads never scan raw JSON.
do $$ declare target uuid; begin
  for target in select id from source.collection_runs
    where metrics->'control_plane'='true'::jsonb
      and (metrics ? 'control_observations' or cursor_after ? 'retry_controls')
    order by started_at,id
  loop perform source.refresh_pncp_contract_query_status(target); end loop;
end $$;

create function api.get_pncp_contract_query_status(control_numbers text[])
returns table(control_number text,state text,checked_at timestamptz,source_url text)
language plpgsql stable security definer set search_path = '' as $$
begin
  if coalesce(cardinality(control_numbers),0)>60 then
    raise exception using errcode='22023', message='Maximum 60 controls'; end if;
  if exists(select 1 from unnest(control_numbers) c where c is null or c !~ '^13654405000195-1-[0-9]{1,12}/[0-9]{4}$') then
    raise exception using errcode='22023', message='Invalid control'; end if;
  return query select c.key,coalesce(q.state,'unknown'),q.checked_at,
    'https://pncp.gov.br/app/editais/13654405000195/'||split_part(c.key,'/',2)||'/'||
      split_part(split_part(c.key,'-',3),'/',1)::bigint::text
  from (select distinct unnest(control_numbers) as key) c
  left join source.pncp_contract_query_status q on q.control_number=c.key;
end $$;
revoke all on function api.get_pncp_contract_query_status(text[]) from public;
grant execute on function api.get_pncp_contract_query_status(text[]) to anon, authenticated;
comment on table source.pncp_contract_query_status is
  'Private latest-query projection. No historical coverage or financial conclusion; executions and artifacts remain evidence.';
