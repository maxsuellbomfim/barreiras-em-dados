...
-- A versão 1.1.0 do parser de regime/vínculo aceita os layouts anteriores do
-- relatório oficial (2021: "Regime" + "Centro de Custo"; 2022 a jun/2025:
-- "Lotação") e recupera a primeira letra do vínculo quando o cabeçalho do PDF
-- está deslocado (jul/2025 a mar/2026). Até aqui só abr a ago/2026 tinham
-- detalhamento publicado; os demais falhavam todo dia. O resultado continua
-- exigindo reconciliação exata com o total oficial; os detalhamentos 1.0.0 já
-- publicados permanecem válidos e não são reprocessados.
...
begin
  ...
  if new.parser_version not in ('payroll-regime-breakdown/1.0.0', 'payroll-regime-breakdown/1.1.0') then
  ...
  where aggregate.report_kind = 'municipal_staff'
    ...
    and not exists (
      ...
      where breakdown.payroll_report_aggregate_id = aggregate.id
        and breakdown.parser_version in ('payroll-regime-breakdown/1.0.0', 'payroll-regime-breakdown/1.1.0')
    )
    ...
    where (select count(*) from current_components)
      = (select count(*)
           ...
           join hr.payroll_report_regime_breakdowns as available
            ...
            and available.parser_version in ('payroll-regime-breakdown/1.0.0', 'payroll-regime-breakdown/1.1.0'))
