"""Benchmark privado e agregado da geometria de valores TCM-BA."""
...
from .tcm_ba_commitment_layout import diagnose_spatial_amount_text


@dataclass(frozen=True)
class TcmBaAmountLayoutTarget:
    artifact: TextArtifact
    candidate_page_counts: tuple[tuple[int, int], ...]


@dataclass(frozen=True)
class TcmBaAmountLayoutBenchmark:
    artifacts: int
...
def benchmark_amount_layout(
    targets: tuple[TcmBaAmountLayoutTarget, ...],
    *,
    ...
    for target in targets:
        ...
        if len(page_counts) != len(target.candidate_page_counts) or any(
            ...
            raise ValueError("O alvo de valor possui contagem de página inválida.")
        ...
        for page_number, candidate_count in page_counts.items():
            ...
            if layout is None or layout.extraction_method != "embedded_layout":
                ...
                continue
            diagnosis = diagnose_spatial_amount_text(layout.blocks)
            ...
            counters[counter] += 1
    return TcmBaAmountLayoutBenchmark(
...
def benchmark_payload(
    ...
    """Produz somente contagens, sem valores, identificadores ou trechos."""
    return {
