"""Mede, sem persistir dotações, a geometria das pendências TCM-BA."""
...
from ..tcm_ba_commitment_budget_diagnostic import (
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Produz somente contagens agregadas do layout de dotações ausentes, "
    ...
    if persistence.mode != "postgres-supabase":
        raise RuntimeError("O benchmark de dotações requer Storage autenticado.")
    if (
    ...
    targets = repository.budget_layout_targets(limit=arguments.limit)
    ...
    exit_code = benchmark_exit_code(
        benchmark,
        expected_candidates=breakdown.missing_budget_allocation,
    )
    payload = benchmark_payload(benchmark)
    payload["expected_candidates"] = breakdown.missing_budget_allocation
    payload["gate"] = "PASS" if exit_code == 0 else "BLOCK"
    log_event(
        logging.getLogger(__name__),
        logging.INFO if exit_code == 0 else logging.ERROR,
        "tcm_ba_commitment_budget_layout_benchmark",
        **payload,
    )
    return exit_code
