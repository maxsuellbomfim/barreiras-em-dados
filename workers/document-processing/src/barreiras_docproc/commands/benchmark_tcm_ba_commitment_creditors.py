"""Mede, sem persistir nomes, a geometria dos credores ausentes TCM-BA."""
...
from ..tcm_ba_commitment_creditor_diagnostic import (
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Produz somente contagens agregadas do layout de credores ausentes, "
            "sem persistir ou registrar nomes."
        )
    )
    ...
    if persistence.mode != "postgres-supabase":
        raise RuntimeError("O benchmark de credores requer Storage autenticado.")
    if (
    ...
    targets = repository.creditor_layout_targets(limit=arguments.limit)
