"""Mede, sem persistir valores, a geometria das pendências TCM-BA."""
...
from ..tcm_ba_commitment_amount_diagnostic import (
    TcmBaAmountLayoutBenchmark,
...
def benchmark_exit_code(
    benchmark: TcmBaAmountLayoutBenchmark,
    *,
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Produz somente contagens agregadas do layout de valores ausentes, "
            "sem persistir ou registrar valores."
        )
    )
    ...
    if persistence.mode != "postgres-supabase":
        raise RuntimeError("O benchmark de valores requer Storage autenticado.")
    if (
    ...
    targets = repository.amount_layout_targets(limit=arguments.limit)
    ...
    exit_code = benchmark_exit_code(
        benchmark,
        expected_candidates=breakdown.missing_amount_text,
    )
    payload = benchmark_payload(benchmark)
    payload["expected_candidates"] = breakdown.missing_amount_text
    payload["gate"] = "PASS" if exit_code == 0 else "BLOCK"
    log_event(
        logging.getLogger(__name__),
        logging.INFO if exit_code == 0 else logging.ERROR,
        "tcm_ba_commitment_amount_layout_benchmark",
        **payload,
    )
    return exit_code
