...
from ..payroll_compensation_publisher import (
    PayrollCompensationPublisher,
    PostgresPayrollCompensationRepository,
)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Publica faixas de provento reconciliadas com a folha regular."
    )
    ...
    repository = PostgresPayrollCompensationRepository.from_dsn(settings.database_url)
    ...
    try:
        for index, artifact in enumerate(artifacts, start=1):
            logger.info(
                "payroll_compensation_start aggregate=%s period=%s progress=%s/%s",
            ...
            try:
                ...
                logger.error(
                    "payroll_compensation_needs_review aggregate=%s error=%s",
                    artifact.aggregate_id,
                    str(error)[:500],
                )
                continue
    ...
    logger.info(
        "payroll_compensation_completed artifacts=%s published=%s "
    ...
    if args.require_complete_month and (
        needs_review or not repository.has_public_distribution(args.reference_month)
    ):
        logger.error(
            "payroll_compensation_incomplete period=%s needs_review=%s",
