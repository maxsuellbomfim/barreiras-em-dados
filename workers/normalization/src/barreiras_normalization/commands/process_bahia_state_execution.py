"""Normaliza o retrato agregado da execucao estadual de emendas."""
...
from ..bahia_state_execution import StateExecutionParseError
from ..bahia_state_execution_processing import (
    ...
    execution_job_idempotency_key,
)
from ..bahia_state_execution_repository import BahiaStateExecutionRepository


@dataclass(frozen=True)
class StateExecutionBatchSummary:
    pending_found: int
    processed: int
    failed: int
    jobs_created: int
...
def _error_code(error: Exception) -> str:
    if isinstance(error, StateExecutionArtifactMismatchError):
        return "artifact_mismatch"
    if isinstance(error, StateExecutionEmptySnapshotError):
        return "empty_snapshot"
    if isinstance(error, StateExecutionParseError):
...
def _error_detail(error: Exception, code: str) -> str:
    safe_messages = {
        ...
        "empty_snapshot": "arquivo verificado sem linhas financeiras",
        "parser_contract": "estrutura oficial incompatível com o parser",
    }
...
def run_batch(
    *,
    ...
    for artifact in pending:
        try:
            ...
            try:
                ...
                log.debug("state_execution_failure_persistence_failed", exc_info=True)
            log_event(
                log,
                logging.ERROR,
                "normalization_bahia_state_execution_failed",
                source="bahia-state-open-data",
        ...
        log_event(
            log,
            logging.INFO,
            "normalization_bahia_state_execution_processed",
            source="bahia-state-open-data",
            artifact_hash=artifact.sha256,
            job_created=result.job_created,
            aggregate_rows=result.results_inserted,
        )

    summary = StateExecutionBatchSummary(
    ...
    log_event(
        log,
        logging.INFO,
        "normalization_bahia_state_execution_batch_completed",
        source="bahia-state-open-data",
        pending_found=summary.pending_found,
        processed=summary.processed,
        failed=summary.failed,
        jobs_created=summary.jobs_created,
        aggregate_rows=summary.results_inserted,
    )
    return summary
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Normaliza a execucao agregada das emendas estaduais, sem atribuir "
            "territorio ausente na fonte oficial."
        )
    )
    ...
    if persistence_settings.mode != "postgres-supabase":
        raise RuntimeError(
            "A normalizacao estadual requer PERSISTENCE_MODE=postgres-supabase."
        )
    if (
    ...
    try:
        ...
        raise RuntimeError(
            "Instale as dependencias opcionais 'postgres' e 'storage'."
        ) from error
    ...
    repository = BahiaStateExecutionRepository.from_dsn(
