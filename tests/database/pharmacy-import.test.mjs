...
test('pharmacy operator import replays without new snapshots, preserves approvals and rolls back conflicts',async()=>{
 const db=new PGlite();
 try {
  await db.exec(`create role anon; create role authenticated; create role service_role;
   ...
   insert into source.source_endpoints(data_source_id,slug,enabled) select id,unnest(array['payment','register']),true from source.data_sources;`);
