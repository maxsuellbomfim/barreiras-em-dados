-- Projeção filtrável do PNCP. Os filtros apenas restringem registros brutos
-- preservados; nenhum valor é recalculado.

drop function if exists api.get_pncp_procurements_filtered(integer, text, smallint, text);

create function api.get_pncp_procurements_filtered(
...
revoke all on function api.get_pncp_procurements_filtered(integer, text, smallint, text) from public;
grant execute on function api.get_pncp_procurements_filtered(integer, text, smallint, text)
  to anon, authenticated;

comment on function api.get_pncp_procurements_filtered(integer, text, smallint, text) is
  'Contratações PNCP filtráveis por fornecedor, ano e texto de objeto/unidade.';
