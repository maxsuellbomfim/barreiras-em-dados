-- ADR 0014: a fila interna precisa explicar por que um item está parado.
-- A migration 20260802200000 corrigiu a deduplicação, mas removeu
-- `assisted_payload` do contrato retornado ao painel. O resultado era uma
-- fila aparentemente sem IA, mesmo quando a sugestão existia no banco.
-- Este contrato é somente leitura, restrito a revisores, e não altera a
...
begin
  ...
  select
    ...
    'extraction-review-queue/1.6.0'::text
  from deduped
...
comment on function api.get_extraction_review_queue(integer) is
  'Fila interna deduplicada, com sugestão assistida e motivo operacional; '
