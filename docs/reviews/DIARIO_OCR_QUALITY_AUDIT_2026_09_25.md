...
- 29 páginas auditadas. A edição 4213/2024 (`referencia=13284`) foi excluída:
  a fonte hoje entrega outro arquivo (SHA-256 `2e189b3c…`) diferente do
  preservado (`8110c515…`). Ver pendências.
- Todas as 29 páginas tinham só o número da página como texto embutido,
...
|---|---|
| Texto corrido (atos, portarias, editais) | Fiel na maioria das páginas, com erros isolados de caractere. Nomes de pessoas e matrículas conferidos sem erro (4071 p.169, 4325 p.7). |
...
1. **Privacidade (prioritária):** mascarar CPF na projeção pública do Diário
   e nos atos, e impedir que a busca encontre documento por CPF.
...
4. **Máscara de CPF da fonte:** tratar como ilegível no texto OCR, e não como
   dado.
5. **4213/2024:** investigar por que `referencia=13284` passou a servir outro
   arquivo. Pode ser correção da prefeitura ou erro de catálogo, como em
   4263/4309.

## Limites

- A amostra tem 29 páginas: estima tipos de erro, não taxa de erro com
