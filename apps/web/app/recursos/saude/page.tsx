...
import { getPharmacyPage, getPharmacyCoverage, getPharmacyRefresh } from '../../../lib/pharmacy';
...
export default async function HealthResourcesPage({searchParams}:{searchParams:Promise<{ano?:string;pagina?:string}>}) {
  ...
  const [{publication,hasNext},coverage,refresh]=await Promise.all([getPharmacyPage(year,page),getPharmacyCoverage(year),getPharmacyRefresh(year)]);
