...
export function CamaraLawsExplorer({ items, totalCount, page, pageSize, initialFilters, authorSummary }: Readonly<{
  ...
  return (
    <div className="acts-explorer">
      ...
      {authorSummary.length > 0 ? <section className="legislative-author-summary" aria-label="Registros por vereador da legislatura atual"><div><strong>Autoria publicada pelos vereadores atuais</strong><span>Somente os 19 nomes da composição atual · contagem global no recorte atual</span></div><div className="legislative-author-bars">{authorSummary.slice(0, 19).map((summary) => <button type="button" key={summary.authorName} onClick={() => { setAuthor(summary.authorName); window.location.assign(authorQuery(summary.authorName)); }} title={`Filtrar por ${summary.authorName}`}><span>{summary.authorName}</span><b style={{ "--bar-size": `${Math.max(8, Math.round((summary.itemCount / authorSummary[0].itemCount) * 100))}%` } as CSSProperties}>{summary.itemCount.toLocaleString("pt-BR")}</b></button>)}</div><p className="act-review-mode">Autores históricos continuam no acervo e podem ser pesquisados, mas não entram neste gráfico por enquanto.</p></section> : null}
