...
} from "./federal-transfer-source-coverage.mjs";

export type FederalTransferSourceCoverageResult =
  | Readonly<{
      state: "available";
...
export async function getPublicFederalTransferSourceCoverage(): Promise<
  FederalTransferSourceCoverageResult
> {
  ...
  const url = `${supabaseUrl}/rest/v1/rpc/get_public_federal_transfer_source_coverage`;
  const headers = {
  ...
  try {
    ...
    const rows = parseFederalTransferSourceCoverageRows(await response.json());
